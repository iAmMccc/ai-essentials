...
当你要报告某个元素「存在」或「不存在」时（特别是分隔线、小字提示、图标等细微元素），先在心里回答：**我在这个位置具体看到了什么？** 是一条灰色细线、一段留白、还是直接接了下一个元素？如果描述不出来，就不要断言有或没有，标注为 `待确认`。
...
完成上述步骤后，**逐条过一遍以下位置**，确认是否遗漏。每个位置都要分别看设计稿和截图中有没有，不一致就是差异：

- [ ] 导航栏底部是否有分隔线
- [ ] 列表最后一项底部是否有分隔线
- [ ] 区块（卡片/section）之间的分隔方式是否一致
- [ ] 底部操作栏顶部是否有分隔线或阴影

不需要在报告中输出这个清单，只是确保你检查过了。
