...
```bash
curl -sL https://raw.githubusercontent.com/iAmMccc/ai-essentials/main/install-skill.sh | bash -s <skill-name>
```

| Skill | 说明 | 安装 |
|-------|------|------|
| [git-commit](skills/git-commit/) | 自动分析 git diff，生成规范的中文 commit message | `bash -s git-commit` |

脚本会自动检测项目中的 `.claude/` 或 `.cursor/` 目录，将 Skill 安装到对应位置。
