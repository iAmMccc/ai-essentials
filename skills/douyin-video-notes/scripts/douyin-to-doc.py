...
def ai_summarize(text, supplements_text, config, comments_text="", video_info=None):
    ...
    prompt = f"""你是一个学习者，刚看完一个视频，现在要把学到的内容整理成笔记。请用第一人称/直接陈述的语气写，不要用第三方转述的语气。

严格遵守以下语气规则：
- ❌ 禁止：「该内容介绍了」「该方法被广泛使用」「作者指出」「视频讲解了」「部分开发者」
- ✅ 正确：「Vibe Coding 从零开始不要凭空让 AI 写」「正确做法是从 GitHub 找成熟项目」「需要注意遵守开源协议」
...
## 摘要
（2-3 句话，不超过 100 字。直接陈述核心观点。）

## 要点
（3-8 条，每条一句话不超过 30 字。直接写知识点，不要「该方法」「该技巧」开头。）

## 正文
（像自己学完后整理的笔记。直接写知识点和操作步骤，不要转述别人说了什么。评论区中有价值的补充（如注意事项）融入正文对应位置，不要单独标注来源。用小标题分段。不要重复摘要和要点。）

## 评论
（保留有实质内容的评论和讨论，忽略纯求资源的。如果都无实质内容则省略。）

注意：
- 全文禁止出现「该」「本视频」「作者」等第三方视角用词
- 摘要、要点、正文不要重复同一个观点
