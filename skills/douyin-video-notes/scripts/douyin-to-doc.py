...
def ai_summarize(text, supplements_text, config, comments_text="", video_info=None):
    ...
    prompt = f"""请根据以下视频文字内容，生成一份学习笔记风格的 Markdown 文档。严格按照以下格式输出：

## 来源
{meta_block}

## 摘要
（2-3 句话，不超过 100 字。用笔记语气直接陈述核心观点，不要用「该内容」「该视频」等第三方视角。）

## 要点
（3-8 条，每条一句话不超过 30 字。提炼关键信息，不要和摘要重复。）

## 正文
（用学习笔记的语气整理，像自己学完后的记录，而不是转述别人的内容。不要用「该方法」「作者指出」「该内容讲解了」这类第三方报道语气。将口语化内容整理为书面表达，去掉语气词和重复，保留专业术语。用小标题分段。评论区中有价值的补充信息（如注意事项、工具推荐）融入正文对应位置。不要重复摘要和要点的内容。）

## 评论
（提取有价值的用户反馈和讨论。保留作者的回复，呈现对话关系。忽略纯求资源、纯表情、无实质内容的评论。如果没有评论或都无实质内容，省略此章节。）
