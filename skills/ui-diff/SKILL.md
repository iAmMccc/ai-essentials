...
不要一次看完整张图就下结论。按以下三步走，漏检率会显著降低。

### 第一步：建立参照体系

先确认两张图的基本状态：
- 两张图分辨率/比例是否一致？不一致先说明，避免把缩放差异当成 UI 问题
- 识别已知尺寸的参照物作为标尺：
...
以这些参照物估算其他元素的间距和尺寸，比纯目测准得多。

### 第二步：分区扫描

把屏幕从上到下分为 3-4 个区域，**逐区对比**：

1. **顶部区域**：导航栏 + 搜索栏 + 页面标题
2. **内容区上半部**：列表/卡片/表单的前半段
3. **内容区下半部**：列表/卡片/表单的后半段
4. **底部区域**：底部按钮 / TabBar / 操作栏

每个区域独立走一遍所有对比维度（间距、颜色、字号、对齐等）。找到差异就记下来，然后继续下一个区域。

**为什么这么做**：一次看整张图时注意力分散，容易漏掉局部细节。分区扫描强制你关注每一块区域。

### 第三步：二次审查

第一遍记录的所有差异，逐条回头验证：
- 这个差异是真实的，还是分辨率缩放造成的视觉误差？
- 这个差异是 UI 问题，还是业务数据不同导致的正常差异？
- 能不能确认，还是只是「感觉不太一样」？

对每条差异标注置信度：
- **确定**：肉眼可见的明显差异（颜色完全不同、元素缺失、布局错乱）
- **高度疑似**：非常可能有差异但无法 100% 确认（间距差 3-5px 级别）
- **待确认**：看不清或无法判断，需要开发核实
